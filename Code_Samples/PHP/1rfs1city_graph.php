...
$query = "select posted, count(posted), (posted - to_date('01-jan-1970','dd-mon-yyyy'))*(24*60*60) from ".$tableName." ";
if (!empty($hood) || !empty($lower) || !empty($upper) || !empty($search) || !empty($date1) || !empty($date2))
{
$query .= "where ";
$addAnd = false;
if (!empty($hood)) {
...
//
$datesCount = floor(($xmax - $xmin) / (24*60*60));
// print "$datesCount days"; exit;

if ($datesCount < 2) {
  print "Less than 2 data points cannot be graphed";
  exit;
}
...
//
// The code to setup a basic graph
//
